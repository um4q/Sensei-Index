...
# -*- coding: utf-8 -*-
"""
Turns one row of the General Equip Install Log into a filled,
ready-to-print PDF, using
General_Electrical_Equipment_Installation_and_Test_Report_TEMPLATE.pdf
as the blank form.

BASIC USE:
    python3 export_general_equip_install_to_pdf.py Electrical_Inspection_Tracker.xlsx --sheet "General Equip Install Log <zone>" --rows 4
...
from general_equip_install_field_map import FIELD_MAP  # noqa: F401
...
def build_values_for_row(ws, field_to_col, row_num):
    ...
    template is a plain text widget (see general_equip_install_field_map.py's
...
def ensure_default_resources(writer):
    ...
    pypdf's field-update code. build_general_equip_install_template.py's
...
def fill_pdf(template_path, values, out_path, flatten=False, add_signature=True):
    ...
    (see general_equip_install_field_map.py's docstring) - so when the
...
def main():
    ...
    tag_col = field_to_col.get("tag_number")
