...
# -*- coding: utf-8 -*-
"""
Turns one row of the Transformer Test Log into a filled, ready-to-print
PDF, using Transformer_Test_Record_TEMPLATE.pdf as the blank form.

BASIC USE:
    python3 export_transformer_test_to_pdf.py Electrical_Inspection_Tracker.xlsx --sheet "Transformer Test Log <zone>" --rows 4
...
from transformer_test_schema import LOG_COLUMNS
from transformer_test_field_map import FIELD_MAP  # noqa: F401

HERE = Path(sys.executable).resolve().parent if getattr(sys, "frozen", False) \
    else Path(__file__).resolve().parent
DEFAULT_TEMPLATE = HERE / "Transformer_Test_Record_TEMPLATE.pdf"
DEFAULT_OUTPUT_DIR = HERE / "output_pdfs"
HEADER_ROW = 3
FIRST_DATA_ROW = 4
SHEET_NAME = "Transformer Test Log"
...
def build_values_for_row(ws, field_to_col, row_num):
    ...
    template is a plain text widget (see transformer_test_field_map.py's
...
def ensure_default_resources(writer):
    ...
    pypdf's field-update code. build_transformer_test_template.py's
...
def fill_pdf(template_path, values, out_path, flatten=False, add_signature=True):
    ...
    (see transformer_test_field_map.py's docstring) - so when the
