# -*- coding: utf-8 -*-
"""
InstINDEX - Access Edition - a PySide6 front end for SenseiIndex.accdb.

    python access_gui_app.py

This is gui_app.py itself, not a rewrite - see README_ACCESS_EDITION.txt
for the full "why a near-verbatim copy" reasoning. The only changes from
gui_app.py are: this docstring, the two `import ... as da`/`import ... as
eda` lines below (pointed at this edition's own Access-backed modules,
which expose the exact same call surface - no explicit connection
argument, same function names/signatures - so every UI class below is
unmodified), APP_TITLE, and a handful of QLabel/QMessageBox strings that
named Excel-only concepts (sheet tabs, "Unhide", the .xlsx file itself)
which don't exist for a database backend - grep this file for "Access
Edition note" to find every one of those spots. Everything else -
every dialog, every page, every keyboard shortcut, the whole visual
design - is byte-for-byte gui_app.py.
...
# access_data_access.py's own import (below) inserts the sibling "Sensei
# Index 2.9" folder onto sys.path as a side effect at module load time -
# that's what makes the next two imports (datasheet_reader/theme, which
# live in THAT folder, not this one) resolve without this file needing
# its own sys.path bootstrapping. Order matters: da must import first.
...
APP_TITLE = "Sensei Index 2.95 - Access Edition"
...
def run_with_lock_retry(parent, action, error_title="Couldn't save"):
    ...
    while True:
        try:
            ...
            # Access Edition note: da.WorkbookLockedError is never
            # actually raised by this edition's own modules (see their
            # own docstrings) - this branch is kept only so the shared
            # retry-loop code stays byte-for-byte identical to gui_app.py.
            reply = QMessageBox.question(
                parent, "Database is locked", str(exc),
...
def remove_series_flow(parent_widget, series_number):
    ...
    warned = QMessageBox.warning(
        ...
        f'Remove "{label}"?\n\n'
        # Access Edition note: this edition does a real relational delete
        # (see access_data_access.py's own remove_series()), not the
        # Excel edition's archive-by-hiding-a-sheet - so the warning says
        # so plainly instead of promising a recovery path that doesn't
        # exist here.
        "This permanently deletes it and every Transmitter/Valve row "
        "under it - there is no archive or Unhide to recover them "
        "afterward. Back up the database first if you're not sure.",
...
def remove_zone_flow(parent_widget, zone_name):
    warned = QMessageBox.warning(
        ...
        f'Remove "{zone_name}"?\n\n'
        # Access Edition note: same real relational delete as
        # remove_series_flow() above - see eda.remove_zone()'s own
        # docstring for why this differs from the Excel edition's
        # archive-by-hiding-a-sheet approach.
        "This permanently deletes it and every Electrical row under it - "
        "there is no archive or Unhide to recover them afterward. Back "
        "up the database first if you're not sure.",
        QMessageBox.Ok | QMessageBox.Cancel, QMessageBox.Cancel)
    if warned != QMessageBox.Ok:
        return
    confirm_text, ok = QInputDialog.getText(
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        drive_btn.setToolTip("Automatic local snapshots of the database - "
    ...
    def add_series(self):
        ...
        show_toast(
            self,
            f"\"{da.series_display_label(number)}\" added.", duration_ms=5000)
        self.refresh_sidebar_and_dashboard()
    ...
    def _build_app_guide_html():
        ...
        parts.append(row("+ Add New Series", "Registers a new series number, ready to take "
                          "Transmitter/Valve rows right away."))
        ...
        parts.append(row("Settings", "Theme, default export options, manage series, "
                          "manage signatures, open the raw database file (Ctrl+,)."))
        ...
        parts.append(row("Refresh", "Reloads from the database on disk (F5) \u2013 use this "
                          "after editing a table directly in Access."))
        ...
        parts.append("<p>For editing many rows at once, the same shortcuts a spreadsheet uses:</p>")
        for keys, desc in [
            ...
            ("Ctrl+V", "paste \u2013 with one row selected, pastes into just that cell; with "
                       ...
                       "multi-cell block \u2013 copied from a spreadsheet, say \u2013 walks it "
        ...
        parts.append("<p>Three ways to pick which rows get turned into PDFs, in order of how "
                      ...
                      "<b>(2) Only rows flagged \u201cExport to PDF = Y\u201d</b> \u2013 "
                      "the row's own long-standing flag field, for anyone who still prefers "
        ...
        parts.append(h("DATABASE INSTRUCTIONS"))
        return "".join(parts)
...
class DashboardPage(QWidget):
    def __init__(self, main_window):
        ...
        export_cleaned_btn.setToolTip("A new .accdb with stray whitespace trimmed and tags upper-cased; "
                                       "duplicate serials and odd-shaped tags are flagged, not touched. "
                                       "The live database itself is never changed.")
    ...
    def _build_tips_card(self):
        ...
        tip = QLabel(
            "The fastest way to log new equipment in bulk is the grid's own "
            "spreadsheet-style paste/fill shortcuts (see Instructions) or "
            "editing a table directly in Access itself. This app's Add New / "
            "Edit forms are best for one-off edits, quick lookups, PDF "
            "exports, and status tracking.")
        tip.setObjectName("StatLabel")
        tip.setWordWrap(True)
        card_layout.addWidget(tip)

        row = QHBoxLayout()
        self.excel_sheets_btn = make_button("\U0001F4C4 View in Access", "Primary")
...
class IndexPage(QWidget):
    ...
    def __init__(self, main_window, series_number, equip_key, filters=None):
        ...
        refresh_btn.setToolTip("Reload from the database (F5)")
    ...
    def reload(self, reselect_row=None):
        try:
            ...
            QMessageBox.critical(self, "Couldn't read database", str(exc))
            self.all_rows = []
    ...
    def remove_selected(self):
        ...
        reply = QMessageBox.question(
            ...
            # Access Edition note: a real DELETE, not a cleared/reusable
            # row the way the Excel edition's own find_first_blank_row()
            # can reuse - see access_data_access.py's own docstring for
            # why there's no "blank row" concept left to free up here.
            "This permanently deletes their rows from the database. It "
            "doesn't touch any other row. Press Ctrl+Z right after if "
            "this was a mistake.")
...
class SettingsDialog(QDialog):
    def __init__(self, parent, on_series_change=None):
        ...
        open_wb_btn = make_button("Open in Access", "Ghost")
        ...
        series_hint = QLabel(
            # Access Edition note: real relational delete, not Excel's
            # archive-by-hiding-a-sheet - see remove_series_flow()'s own
            # note above for why this can't say "nothing is ever
            # permanently destroyed" the way gui_app.py's own hint does.
            "Rename gives a series a friendlier label everywhere it's shown. "
            "Remove permanently deletes it and its rows - there is no "
            "archive to recover them from.")
    ...
    def _open_workbook(self):
        try:
            ...
            QMessageBox.critical(self, "Couldn't open database", str(exc))
...
class BackupsDialog(QDialog):
    ...
    def __init__(self, parent, backend=None, backups_dir=None, workbook_label="Instrumentation"):
        ...
        info = QLabel(
            ...
            # Access Edition note: Instrumentation and Electrical share ONE
            # SenseiIndex.accdb here (not two separate workbooks the way
            # the Excel edition's own Equipment_Inspection_Tracker.xlsx/
            # Electrical_Inspection_Tracker.xlsx are), so "domains" fits
            # better than "workbooks" - the settings sharing itself is
    ...
    def _reload(self):
        ...
        if not self._backups:
            ...
            none_item = QTableWidgetItem("No backups yet - they start once the database is first saved.")
    ...
    def _restore_selected(self):
        ...
        text, ok = QInputDialog.getText(
            self, "Restore backup",
            f"This replaces the current, LIVE database with:\n\n{backup['name']}\n\n"
...
class ExportDialog(QDialog):
    def __init__(self, parent, series_number, equip_key, filters=None):
        ...
        folder_hint = QLabel("Files always land inside output_pdfs/ next to the database. "
...
class MasterListImportDialog(QDialog):
    ...
    def _refresh_step3_summary(self):
        ...
        lines.append(
            ...
            "Master List file and your tracker database are both untouched.")
        self.step3_summary.setText("\n".join(lines))
...
class PopulatingWizardDialog(QDialog):
    ...
    def _save_draft_and_close(self):
        ...
        QMessageBox.information(
            ...
            "already saved this session stay in the database as-is. This batch's setup and "
    ...
    def reject(self):
        if self.stack.currentWidget() is self.page_entry and self._entry_form_has_content():
            reply = QMessageBox.question(
                ...
                f"{self.entries_this_session} entr{'y' if self.entries_this_session == 1 else 'ies'} "
                "already saved this session will stay in the database. Anything currently "
...
class ElectricalEditDialog(QDialog):
    ...
    def save(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Database is locked", str(exc))
            return
        except Exception as exc:
            QMessageBox.critical(self, "Couldn't save", str(exc))
            return
...
class ElectricalIndexPage(QWidget):
    ...
    def remove_selected(self):
        ...
        reply = QMessageBox.question(
            self, "Remove rows",
            f"Remove {len(row_nums)} row(s)? ({preview})\n\nThis permanently deletes their rows "
            "from the database. Press Ctrl+Z right after if this was a mistake.")
        if reply != QMessageBox.Yes:
            return
