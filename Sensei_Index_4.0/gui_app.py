...
class MainWindow(QMainWindow):
    ...
    def refresh_current_view(self):
        ...
        reflects it. Deliberately looks up 'whatever page is showing RIGHT
        NOW' rather than having the undo/redo closures capture the page
        that was showing at the time of the original action - that page
...
class IndexPage(QWidget):
    ...
    def _build_status_widget(self, row_num, key_value, field, checked):
        ...
        # A screen reader landing on a bare checkbox in a grid only ever
        # announces the column ("Submitted, checkbox") - it can't say which
        # ROW, since the checkbox itself carries no text. Naming it
        # "<tag>, <column>" makes each one announce as its own sentence,
        # same as every other accessible name in this app after the 3.0
        # redesign pass.
        box.setAccessibleName(f"{key_value}, {self.STATUS_LABELS[field]}")
