...
# ---------------------------------------------------------------------------
# The Run view - one flat, cross-series, cross-equipment-type list, driven
# by Sensei Index 4.0's "Run" screen design: a single fast-moving worklist
# instead of navigating series-by-series and type-by-type through the
# sidebar. Everything here is read/write through the exact same
# read_index_rows_with_status() / set_status() functions the sidebar-driven
# Index view already uses - stage 0-3 IS installed/submitted/accepted,
# just exposed as one number so the Run view's segmented stage control has
# a single source of truth to move forward and back through.
# ---------------------------------------------------------------------------
...
def get_run_stage(series_number, equip_key, key_value):
    return stage_from_status(get_status(series_number, equip_key, key_value))


def set_run_stage(series_number, equip_key, key_value, stage):
    """stage 0-3. Setting stage=2 ('Submitted') sets installed=True,
    submitted=True, accepted=False in one call - moving forward always
    implies every earlier stage happened too, same as ticking Installed
    then Submitted by hand already meant."""
